...
(async () => {
  ...
  await page.goto('http://localhost:8000/assessment', { waitUntil: 'domcontentloaded' });
  await page.evaluate((token) => {
    localStorage.setItem('token', token);
  }, TOKEN);
  await page.goto('http://localhost:8000/assessment', { waitUntil: 'networkidle0' });
  await wait(3000);
  
  const content = await page.evaluate(() => document.body.innerText);
  console.log('Page content preview:', content.substring(0, 300));
  
  await page.screenshot({ path: '/workspace/screenshots/assessment.png' });
  console.log('Saved:', fs.statSync('/workspace/screenshots/assessment.png').size, 'bytes');
